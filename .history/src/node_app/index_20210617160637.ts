...
export class App {
  ...
  constructor() {
    ...
    this.endSessionHandler = new NodeBasedHandlerEndSession(PORT);
  ...
  makeAuthorizationRequest(configuration: AuthorizationServiceConfiguration) {
    ...
    this.makeEndSessionRequest(configuration: EndSessionServiceConfiguration);
  }

  makeEndSessionRequest(configuration: EndSessionServiceConfiguration) {
    // create a request
    let request = new EndSessionRequest({
      client_id: clientId,
      redirect_uri: redirectUri,
      scope: scope,
      response_type: EndSessionRequest.RESPONSE_TYPE_CODE,
      state: undefined,
      extras: { 'prompt': 'consent', 'access_type': 'offline' }
    }, new NodeCrypto());
